...
def get_user_list():
    ...
    sex = request.json.get("sex", "0").strip()  # 性别，默认为0
    role = request.json.get("telephone", "").strip()  # 手机号

    sql = "SELECT * FROM user WHERE username = '{}' and telephone='{}' and sex='{}' and role='{}'".format(username, telephone, sex, role)
    data = db.select_db(sql)
    print("获取 {} 用户信息 == >> {}".format(username, data))
    if data:
        return jsonify({"code": 0, "data": data, "msg": "查询成功"})
