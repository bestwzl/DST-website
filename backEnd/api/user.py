from flask import Flask, jsonify, request
from common.mysql_operate import db
from common.redis_operate import redis_db
from common.md5_operate import get_md5
import re, time

from flask import Blueprint

user = Blueprint("users", __name__)

# app = Flask(__name__)
# app.config["JSON_AS_ASCII"] = False  # jsonify返回的中文正常显示


@user.route('/')
def hello_world():
    return 'Hello World!'


@user.route("/users", methods=["GET"])
def get_all_users():
    """获取所有用户信息"""
    sql = "SELECT * FROM users"
    data = db.select_db(sql)
    print("获取所有用户信息 == >> {}".format(data))
    return jsonify({"code": 0, "data": data, "msg": "查询成功"})


@user.route("/users/<string:name>", methods=["GET"])
def get_user(name):
    """获取某个用户信息"""
    sql = "SELECT * FROM users WHERE name = '{}'".format(name)
    data = db.select_db(sql)
    print("获取 {} 用户信息 == >> {}".format(name, data))
    if data:
        return jsonify({"code": 0, "data": data, "msg": "查询成功"})
    return jsonify({"code": "1004", "msg": "查不到相关用户的信息"})



@user.route("/getUsers", methods=["POST"])
def get_user_list():
    """获取某个用户信息"""
    name = request.json.get("name", "").strip()  # 用户名
    account = request.json.get("account", "").strip()  # 账户
    password = request.json.get("sex", "0").strip()  # 密码
    role = request.json.get("role", "").strip()  # 类型

    sql = "SELECT * FROM users WHERE name = '{}' and account='{}' and password='{}' and role='{}'".format(name, account, password, role)
    data = db.select_db(sql)
    print("获取 {} 用户信息 == >> {}".format(name, data))
    if data:
        return jsonify({"code": 0, "data": data, "msg": "查询成功"})
    return jsonify({"code": "1004", "msg": "查不到相关用户的信息"})




@user.route("/register", methods=['POST'])
def user_register():
    """注册用户"""
    name = request.json.get("name", "").strip()  # 用户名
    password = request.json.get("password", "").strip()  # 密码
    # sex = request.json.get("sex", "0").strip()  # 性别，默认为0(男性)
    account = request.json.get("account", "").strip()  # 手机号
    role = request.json.get("role", "").strip()  # 地址，默认为空串
    if name and password and account: # 注意if条件中 "" 也是空, 按False处理
        sql1 = "SELECT name FROM users WHERE name = '{}'".format(name)
        res1 = db.select_db(sql1)
        print("查询到用户名 ==>> {}".format(res1))
        sql2 = "SELECT account FROM users WHERE account = '{}'".format(account)
        res2 = db.select_db(sql2)
        print("查询到账号 ==>> {}".format(res2))
        if res1:
            return jsonify({"code": 2002, "msg": "用户名已存在，注册失败！！！"})
        # elif not (sex == "0" or sex == "1"):
        #     return jsonify({"code": 2003, "msg": "输入的性别只能是 0(男) 或 1(女)！！！"})
        # elif not (len(telephone) == 11 and re.match("^1[3,5,7,8]\d{9}$", telephone)):
        #     return jsonify({"code": 2004, "msg": "手机号格式不正确！！！"})
        elif res2:
            return jsonify({"code": 2005, "msg": "账号已存在！！！"})
        else:
            password = get_md5(name, password) # 把传入的明文密码通过MD5加密变为密文，然后再进行注册
            sql3 = "INSERT INTO users(name, password, role, account) " \
                  "VALUES('{}', '{}', '{}', '{}')".format(name, password, account, role)
            db.execute_db(sql3)
            print("新增用户信息SQL ==>> {}".format(sql3))
            return jsonify({"code": 0, "msg": "恭喜，注册成功！"})
    else:
        return jsonify({"code": 2001, "msg": "用户名/密码/账号不能为空，请检查！！！"})


@user.route("/login", methods=['POST'])
def user_login():
    """登录用户"""
    name = request.values.get("name", "").strip()
    password = request.values.get("password", "").strip()
    if name and password: # 注意if条件中空串 "" 也是空, 按False处理
        sql1 = "SELECT name FROM users WHERE name = '{}'".format(name)
        res1 = db.select_db(sql1)
        print("查询到用户名 ==>> {}".format(res1))
        if not res1:
            return jsonify({"code": 1003, "msg": "用户名不存在！！！"})
        md5_password = get_md5(name, password) # 把传入的明文密码通过MD5加密变为密文
        sql2 = "SELECT * FROM users WHERE name = '{}' and password = '{}'".format(name, md5_password)
        res2 = db.select_db(sql2)
        print("获取 {} 用户信息 == >> {}".format(name, res2))
        if res2:
            timeStamp = int(time.time()) # 获取当前时间戳
            # token = "{}{}".format(username, timeStamp)
            token = get_md5(name, str(timeStamp)) # MD5加密后得到token
            redis_db.handle_redis_token(name, token) # 把token放到redis中存储
            login_info = { # 构造一个字段，将 id/username/token/login_time 返回
                "id": res2[0]["id"],
                "username": name,
                "token": token,
                "login_time": time.strftime("%Y/%m/%d %H:%M:%S")
            }
            return jsonify({"code": 0, "login_info": login_info, "msg": "恭喜，登录成功！"})
        return jsonify({"code": 1002, "msg": "用户名或密码错误！！！"})
    else:
        return jsonify({"code": 1001, "msg": "用户名或密码不能为空！！！"})

@user.route("/update/users/<int:id>", methods=['POST'])
def user_update(id): # id为准备修改的用户ID
    """修改用户信息"""
    admin_user = request.json.get("admin_user", "").strip() # 当前登录的管理员用户
    token = request.json.get("token", "").strip()  # token口令
    new_password = request.json.get("password", "").strip()  # 新的密码
    new_name = request.json.get("name", "").strip()  # 新的性别，如果参数不传sex，那么默认为0(男性)
    new_account = request.json.get("telephone", "").strip()  # 新的手机号
    new_role= request.json.get("address", "").strip()  # 新的联系地址，默认为空串
    if admin_user and token and new_password and new_account: # 注意if条件中空串 "" 也是空, 按False处理
        # if not (new_sex == "0" or new_sex == "1"):
        #     return jsonify({"code": 4007, "msg": "输入的性别只能是 0(男) 或 1(女)！！！"})
        # elif not (len(new_telephone) == 11 and re.match("^1[3,5,7,8]\d{9}$", new_telephone)):
        #     return jsonify({"code": 4008, "msg": "手机号格式不正确！！！"})
        # else:
            redis_token = redis_db.handle_redis_token(admin_user) # 从redis中取token
            if redis_token:
                if redis_token == token: # 如果从redis中取到的token不为空，且等于请求body中的token
                    sql1 = "SELECT role FROM users WHERE name = '{}'".format(admin_user)
                    res1 = db.select_db(sql1)
                    print("根据用户名 【 {} 】 查询到用户类型 == >> {}".format(admin_user, res1))
                    user_role = res1[0]["role"]
                    if user_role == 0: # 如果当前登录用户是管理员用户
                        sql2 = "SELECT * FROM users WHERE id = '{}'".format(id)
                        res2 = db.select_db(sql2)
                        print("根据用户ID 【 {} 】 查询到用户信息 ==>> {}".format(id, res2))
                        sql3 = "SELECT account FROM users WHERE account = '{}'".format(new_account)
                        res3 = db.select_db(sql3)
                        print("返回结果：{}".format(res3))
                        print("查询到账号 ==>> {}".format(res3))
                        if not res2: # 如果要修改的用户不存在于数据库中，res2为空
                            return jsonify({"code": 4005, "msg": "修改的用户ID不存在，无法进行修改，请检查！！！"})
                        elif res3: # 如果要修改的手机号已经存在于数据库中，res3非空
                            return jsonify({"code": 4006, "msg": "账号已被注册，无法进行修改，请检查！！！"})
                        else:
                            # 如果请求参数不传address，那么address字段不会被修改，仍为原值
                            if not new_role:
                                new_address = res2[0]["address"]
                            # 把传入的明文密码通过MD5加密变为密文
                            new_password = get_md5(res2[0]["username"], new_password)
                            sql3 = "UPDATE users SET password = '{}', account = '{}', role = '{}', name = '{}' " \
                                   "WHERE id = {}".format(new_password, new_account, new_name, new_role, id)
                            db.execute_db(sql3)
                            print("修改用户信息SQL ==>> {}".format(sql3))
                            return jsonify({"code": 0, "msg": "恭喜，修改用户信息成功！"})
                    else:
                        return jsonify({"code": 4004, "msg": "当前用户不是管理员用户，无法进行操作，请检查！！！"})
                else:
                    return jsonify({"code": 4003, "msg": "token口令不正确，请检查！！！"})
            else:
                return jsonify({"code": 4002, "msg": "当前用户未登录，请检查！！！"})
    else:
        return jsonify({"code": 4001, "msg": "管理员用户/token口令/密码/手机号不能为空，请检查！！！"})

@user.route("/delete/user/<string:name>", methods=['POST'])
def user_delete(name):
    admin_user = request.json.get("admin_user", "").strip()  # 当前登录的管理员用户
    token = request.json.get("token", "").strip()  # token口令
    if admin_user and token:
        redis_token = redis_db.handle_redis_token(admin_user)  # 从redis中取token
        if redis_token:
            if redis_token == token:  # 如果从redis中取到的token不为空，且等于请求body中的token
                sql1 = "SELECT role FROM users WHERE name = '{}'".format(admin_user)
                res1 = db.select_db(sql1)
                print("根据用户名 【 {} 】 查询到用户类型 == >> {}".format(admin_user, res1))
                user_role = res1[0]["role"]
                if user_role == 0:  # 如果当前登录用户是管理员用户
                    sql2 = "SELECT * FROM users WHERE name = '{}'".format(name)
                    res2 = db.select_db(sql2)
                    print(sql2)
                    print("根据用户名 【 {} 】 查询到用户信息 ==>> {}".format(name, res2))
                    if not res2:  # 如果要删除的用户不存在于数据库中，res2为空
                        return jsonify({"code": 3005, "msg": "删除的用户名不存在，无法进行删除，请检查！！！"})
                    elif res2[0]["role"] == 0: # 如果要删除的用户是管理员用户，则不允许删除
                        return jsonify({"code": 3006, "msg": "用户名：【 {} 】，该用户不允许删除！！！".format(name)})
                    else:
                        sql3 = "DELETE FROM users WHERE name = '{}'".format(name)
                        db.execute_db(sql3)
                        print("删除用户信息SQL ==>> {}".format(sql3))
                        return jsonify({"code": 0, "msg": "恭喜，删除用户信息成功！"})
                else:
                    return jsonify({"code": 3004, "msg": "当前用户不是管理员用户，无法进行操作，请检查！！！"})
            else:
                return jsonify({"code": 3003, "msg": "token口令不正确，请检查！！！"})
        else:
            return jsonify({"code": 3002, "msg": "当前用户未登录，请检查！！！"})
    else:
        return jsonify({"code": 3001, "msg": "管理员用户/token口令不能为空，请检查！！！"})